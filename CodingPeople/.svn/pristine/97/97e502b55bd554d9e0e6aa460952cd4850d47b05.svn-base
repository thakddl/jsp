...
@WebServlet("/groupStudy/list2.do")
public class GroupStudyList extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 2. 쿼리 실행하여 list에 담기
		List<GroupStudyVO> gsList = groupStudyService.getGroupStudyList();
		
		// 3. 리퀘스트 객체에 list 담기
		req.setAttribute("gsList", gsList);
		
		// 4. 디스패쳐에 리퀘스트 객체 담기
		RequestDispatcher dispatcher = req.getRequestDispatcher("/groupStudy/GroupStudyList.jsp");
