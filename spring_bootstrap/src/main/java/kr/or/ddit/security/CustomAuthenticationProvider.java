...
public class CustomAuthenticationProvider implements AuthenticationProvider {
	...
	private LoginSearchMemberService memberServce;
	...
	public Authentication authenticate(Authentication auth) throws AuthenticationException {
		...
		try {
			member = memberServce.getMember(login_id);
			...
			if (member != null) {
				if (login_pwd.equals(member.getPwd())) {// 아이디 패스워드 일치
					...
					throw new BadCredentialsException("상태변경으로 로그인이 불가능 합니다.");
				} else {// 패스워드 불일치
		...
		} catch (Exception e) {// SQLException인데 throws AuthenticationException이라(정해짐) security가 만든 Exception만
								// 내보낼수잇잖아.
			e.printStackTrace();
